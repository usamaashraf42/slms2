					...
					<div class="heading-elements float-right">
						<a href="{{route('product.create')}}">
							<button type="button" class="btn btn-success btn-min-width mr-1 mb-1">
								<i class="la la-plus">&nbsp;Add Product</i>
							</button>
						</a> 
					</div>
                  ...
                  <th>Price</th>
							...
							<tbody>
								...
								<tr>
									...
									<td>
                    <a class="example-image-link" href="javascript:;" data-lightbox="example-set" data-title="Click the right half of the image to move forward.">
                        <img class="example-image" src="{{asset('images/Inventory/product/'.$category->images)}}"  alt="{{$category->pro_name}}" height="60" width="60" style="border-radius: 50%!important;" />
                    </a>
                  </td>
                  <td><strong>{{$category->pro_name}}</strong></td>
                  <td>@isset($category->category){{$category->category->pro_cat_name}}@endisset</td>
                  <td>@isset($category->sub_category){{$category->sub_category->pro_cat_name}}@endisset</td>
                  <td>{{$category->price}}</td>
									<td>
										@if($category->status)
										<a href="javascript:;" onclick="productCategoryDelete(this,{{$category->pro_id}})"  class="btn btn-success btn-sm">Active</a> 
										@else 
										<a href="javascript:;" onclick="productCategoryDelete(this,{{$category->pro_id}})" class="btn btn-warning btn-sm">Deactive</a> 
										@endif
										
									</td>
										
										<td><a href="{{route('product.edit',$category->pro_id)}}"  class="btn btn-warning btn-sm">Edit</a></td>
									</tr>
									@endforeach
									@endisset
								</tbody>
