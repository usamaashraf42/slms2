          ...
          <div class="table-responsive">
            <table id="" class="table border table-bordered ">
              ...
              <tbody>
                ...
                <tr>
                  <input type="hidden" name="emp_ids[]" value="@isset($admin->employee_id){{$admin->employee_id}}@endisset">
                  <input type="hidden" name="month" value="{{$month}}">
                  <input type="hidden" name="year" value="{{$year}}">
                  <td>{{$counter++}}</td>
                  <td>@isset($admin->employee_id){{$admin->employee_id}}@endisset</td>
                  <td>@isset($admin->employee->name){{$admin->employee->name}}@endisset</td>
                  <td>@isset($admin->current_salay){{$admin->current_salay}}@endisset</td>

                  <td>{{$admin->total_days}}</td>
                  <td>{{$admin->total_absent}}</td>
                  <td>{{$admin->total_leaves}}</td>
                  <td>{{0}}</td>
                  <td>{{$admin->total_present}}</td>
                  <td>{{ $admin->totalpaydays}}</td>
                  
                  <td>{{$admin->EOBI}}</td>
                  <td>{{$admin->advance_deduction}}</td>
                  <td>{{$admin->security}}</td>
                  <td>{{$admin->tax}}</td>
                  <td>{{$admin->ta}}</td>
                  <td>{{round($admin->pf,2)}}</td>
                  <td>{{round($admin->tbPF,2)}}</td>
                  <td>{{ round(($admin->actualSalaryGiven ),2) }}</td>
                </tr>
                @endforeach
                @endisset
              </tbody>
