				...
				<div class="card-block">
					...
					<div class="table-responsive">
						<table id="example" class="table border table-bordered ">
							...
							<tbody>
								@isset($category)
									@foreach($category as $sec)
									<tr>
										<td><input type="checkbox" name="ids[]" value="{{$sec->id}}"></td>
										<td>@isset($sec->cat_name){{$sec->cat_name}}@endisset</td>
										<td>@isset($sec->description){{$sec->description}}@endisset</td>
...
<script>
	...
	function deleteItem(id) {
		...
		if (val) {
			$.ajax({
				url: "{{route('student-category.deleteCategory')}}",
				type: 'post',
				data: {
					'id': id
				},
	...
	function editItem(id) {
		$.ajax({
			url: 'admin/student-category/'+id+'/edit',
			type: 'get',
			data: {
				'id': id
			},
	...
	function detail(id) {
        ...
        $("#addDataBtn").click(function (e) {
            ...
            $.ajax({
                url: "{{route('student-category.store')}}",
                type: "POST",
