...
@section('title', 'Evolution Indicator ')
@section('content')
		...
		<div class="col-md-12 col-sm-12 col-lg-12 col-xl-12">
			<div class="card-box">
				...
				<div class="card-block">
					<h4 class="card-title">Evolution Indicator</h4>
					<div class="heading-elements float-right">
						<a href="javascript:;">
							<button type="button" class="btn add-btn btn-success" data-toggle="modal" data-target="#add_department">
								<i class="la la-plus">&nbsp;Add Evolution Indicator</i>
							</button>
						</a> 
					</div>
				</br>
				...
				<div class="table-responsive">
					<table id="example" class="table border table-bordered ">
						...
						<tbody>
							@isset($performances)
							@foreach($performances as $admin)
							<tr>
								<td><input type="checkbox" name="ids[]" value="{{$admin->id}}"></td>
								<td>@isset($admin->indicator_name){{$admin->indicator_name}}@endisset</td>
								<td>@isset($admin->indicator_total_marks){{$admin->indicator_total_marks}}@endisset</td>


								<td>
									@if($admin->status)
									<a href="javascript:;" onclick="deleteItem({{$admin->id}})"  class="btn btn-success btn-sm">Active</a> 
									@else 
									<a href="javascript:;" onclick="deleteItem({{$admin->id}})" class="btn btn-warning btn-sm">Deactive</a> 
									@endif


								</td>
								<td><a href="javascript:;" onclick="editItem({{$admin->id}})"  class="btn btn-success btn-sm">Edit</a></td>
							</tr>
							@endforeach
							@endisset
						</tbody>
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Add Evolution Indicator </h5>
			...
			<div class="modal-body">
				<form method="POST" action="{{route('performance-indicator.store')}}">
					...
					<div class="form-group">
						<label for="indicator_name">Evolution Indicator  <span class="text-danger">* </span></label>
	...
	function editItem(id) {
		$.ajax({
			...
			success: function (response) {
				...
				$("#edit_performanceIndicator").modal('show');
			},
			error: function (e) {
				console.log('error', e);
			}
