  ...
  <div class="row">
       ...
       <thead>
        ...
        <tr>
          ...
          <th> <div style="transform: rotate(90deg);padding: 0px; max-width: 35px; font-size: 8pt; font-weight: bold;"> insurance</div></th>
           <th ><div style="transform: rotate(90deg);padding: 0px; max-width: 37px; font-size: 8pt; font-weight: bold;"  >Admission / Registeration </div></th>
    ...
    <tbody>
        ...
        <td style="text-align: left!important;"><i>{{$data->s_name}}&nbsp; &nbsp; {{$data->s_phoneNo}}</td>
          <td style="text-align: left!important;"><i>@isset($data->id){{$data->id}}@endisset</td>
            ...
            <td>@isset($studentFeeMonth->insurance_of) {{$studentFeeMonth->insurance_of}}@endisset </td>
             <td>@isset($studentFeeMonth->admissionFee)
              @php($admission+=$studentFeeMonth->admissionFee)
    
              {{($studentFeeMonth->admissionFee)}}
              @endisset / @isset($studentFeeMonth->registerationFee)
              @php($register+=$studentFeeMonth->registerationFee)
              {{($studentFeeMonth->registerationFee)}}
            @endisset</td>
            ...
            <td>@if($thisMonthPaid < $studentFeeMonth->paid_amount)
              {{($studentFeeMonth->paid_amount-$studentFeeMonth->registerationFee-$studentFeeMonth->admissionFee)- ($thisMonthPaid)}}
              @php($totalPayableLast+=($studentFeeMonth->paid_amount-$studentFeeMonth->registerationFee-$studentFeeMonth->admissionFee)- ($thisMonthPaid))
            @endisset</td>
          </tr>
      ...
      <table class="table_1">
        ...
        <tr>
          <td>This Month Fee Collected</td>
          <td>{{$totalPaidCurrent}}</td>
        </tr>
        <tr>
          <td>Outstanding Fee Collected</td>
          <td>{{$totalPayableLast}}</td>
        </tr>
        <tr>
          <td>Total Student Paid</td>
          <td>{{$counter}}</td>
        </tr>
        <tr>
          <td>Total Student Unpaid</td>
          <td>{{count($branch->student)-$counter}}</td>
        </tr>
      </table>
      </div>
      </div>
