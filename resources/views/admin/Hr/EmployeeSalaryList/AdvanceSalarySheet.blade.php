					...
					<div style="width: 100%;">
						...
						<page  size="A4" layout="landscape" style="page-break-after: always!important;padding: 20px;clear: both!important;">
							<div class="logo_heading" style="margin: 0 auto; width: 400px;padding-top: 30px;">
								...
								<p style="text-align: right;">Employee Salary Sheet of  {{ isset($month)? getMonthName($month) : ''}}-{{$year}}</p>
							</div>
								...
								<table style="width: 100%;">
									<thead>
										...
										<tbody id="dataTable">
											...
											<tr >
												...
												<td>
													@isset($data->Employeesalary) 
														@php($totalSalary+=$data->Employeesalary->monthly_salary)
														{{ $data->Employeesalary->monthly_salary }} 
													@endisset
												</td>
												<td>
													@isset($payroll->pf) 
														@php($totalPf +=($data->Employeesalary->monthly_salary * $payroll->pf)/100 ) 
														{{ ($data->Employeesalary->monthly_salary * $payroll->pf)/100 }} 
													@endisset</td>
												<td>
													@isset($payroll->eobi)  
														@php($totalEobi+=  ($data->Employeesalary->monthly_salary * $payroll->eobi)/100) 
														{{ ($data->Employeesalary->monthly_salary * $payroll->eobi)/100 }} 
													@endisset</td>
									...
									<div style="width: 100%;">
										<div style="width:45%;margin-top: 20px;">
											<p style="padding-left: 30px;">Total Employee : <strong>{{$empCounter-1}}</strong> </p>
										</div>
