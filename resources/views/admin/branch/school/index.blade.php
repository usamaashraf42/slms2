				...
				<div class="card-block">
					...
					<div class="heading-elements float-right">
						<a href="{{route('school.create')}}">
							<button type="button" class="btn btn-success btn-min-width mr-1 mb-1">
								<i class="la la-plus">&nbsp;Add School</i>
							</button>
						</a> 
					</div>
					...
					@endcomponent
						<table id="example" class="table border table-bordered ">
							...
							<tbody>
								@isset($schools)
								@foreach($schools as $admin)
								<tr>
									<td><input type="checkbox" name="ids[]" value="{{$admin->id}}"></td>
									<td>@isset($admin->school_name){{$admin->school_name}}@endisset</td>
									<td>@isset($admin->sch_cheif_name){{$admin->sch_cheif_name}}@endisset</td>
									
									<td>@isset($admin->school_phone_no){{$admin->school_phone_no}}@endisset</td>

									<td>@isset($admin->sch_tel_no){{$admin->sch_tel_no}}@endisset</td>
									<td>@if(isset($admin->branch) && count($admin->branch)) {{schoolbranchImplode($admin->branch)}} @endif</td>
									<td>@isset($admin->address){{$admin->address}}@endisset</td>

									<td>
										@if($admin->school_status)
										...
										<td><a href="{{route('school.edit',$admin->id)}}"  class="btn btn-info btn-sm">Edit</a></td>
									</tr>
									@endforeach
									@endisset
								</tbody>
