			...
			<div class="table-responsive">
				<table class="table table-striped custom-table mb-0">
					...
					<tbody>
						...
						<tr class="@if($holiday->holiday_date < date('Y-m-d') ) holiday-completed @else holiday-upcoming @endif">
							<td>{{$holiday->id}} </td>
							<td>{{$holiday->holiday_title}} </td>
							<td>{{  date("d-M-y", strtotime($holiday->holiday_date))  }} </td>
							<td>{{date("D", strtotime($holiday->holiday_date))}} </td>
							<td>

								
								
								@if($holiday->holiday_date > date('Y-m-d') ) 
			...
			<div class="modal-body">
				<form method="POST" action="{{route('holidays.store')}}">
					...
					<div class="form-group">
						...
						<input class="form-control" name="holiday_title" type="text" />
					</div>
					<div class="form-group">
