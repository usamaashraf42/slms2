		...
		<div class="col-md-12 col-sm-12 col-lg-12 col-xl-12">
			<div class="card-box">
				...
				<div class="card-block">
					<h4 class="card-title">HomeWork Mangement</h4>
					<div class="heading-elements float-right">
						<a href="{{route('home-work.create')}}">
							<button type="button" class="btn btn-success btn-min-width mr-1 mb-1">
								<i class="la la-plus">&nbsp;Add HomeWork</i>
							</button>
						</a> 
					</div>
					...
					<div class="table-responsive">
						<table id="" class="table border table-bordered ">
							...
							<tbody>
								@isset($homeworks)
								@foreach($homeworks as $admin)
								<tr>
									<td><input type="checkbox" name="ids[]" value="{{$admin->id}}"></td>
									<td>@isset($admin->name){{$admin->name}}@endisset</td>
									<td>@isset($admin->email){{$admin->email}}@endisset</td>
									
									<td>@isset($admin->branch){{$admin->branch->branch_name}}@endisset</td>
									<td>@isset($admin->roles){{roleImplode($admin->roles)}}@endisset</td>
									<td>@isset($admin->phone){{$admin->phone}}@endisset</td>
									<td>
										@if(!(Auth::user()->branch_id))
										@if($admin->activity)
										<a href="javascript:;" onclick="Deactive({{$admin->id}})"  class="btn btn-success btn-sm">Approved</a> 
										@else 
										<a href="javascript:;" onclick="activeAccount({{$admin->id}})" class="btn btn-warning btn-sm">Unapproved</a> 
										@endif
										@endif
										...
										<!-- <td>
											@if(!(Auth::user()->branch_id))
											@if($admin->status)
												<a href="javascript:;" onclick="deleteItem({{$admin->id}})"  class="btn btn-success btn-sm">verified</a> 
											@else 
												<a href="javascript:;" onclick="deleteItem({{$admin->id}})" class="btn btn-warning btn-sm">Not verify</a> @endif
											@endif
												
										</td> -->
										<td><a href="{{route('staff.edit',$admin->id)}}"  class="btn btn-info btn-sm">Edit</a></td>
									</tr>
									@endforeach
									@endisset
								</tbody>
