		...
		<div class="col-md-12 col-sm-12 col-lg-12 col-xl-12">
			<div class="card-box">
				<div class="card-block">
					...
					@endcomponent
					<form method="POST" action="{{ route('account-category.store') }}" enctype = "multipart/form-data" id="upload_new_form">
						<div class="col-md-12">
							{{csrf_field()}}
							...
							<div class="col-md-10">
								<div class="form-group">
									<label for="exampleInputPassword1">Account Category Name</label>
									    <input type="text" class="form-control category" value="{{old('name')}}" name="name" id="name" placeholder="Account Category Name">
								</div>
							</div>
							<div class="row">
								<div class="col-md-4">
									<div class="form-group @if($errors->has('parent_id')) has-error @endif"">
										...
										<select  class="form-control " value="{{old('parent_id')}}" name="parent_id" id="parent_id"  placeholder="">
											...
											@foreach($categories as $cat)
												<option value="{{$cat->id}}" @if($cat->id==old('parent_id')) selected @endif>{{$cat->name}}</option>
											@endforeach
									...
									<div class="form-group @if($errors->has('class')) has-error @endif"">
										...
										<select  class="form-control " value="{{old('class')}}" name="class" id="class"  placeholder="">
											<option disabled="disabled" selected="selected">Choose the Class</option>
												...
												<option value="asset" @if('asset'==old('class')) selected @endif>Asset</option>
												<option value="liability" @if('liability'==old('class')) selected @endif>Liability</option>
												<option value="expense" @if('expense'==old('class')) selected @endif>Expense</option>
												<option value="revenue" @if('revenue'==old('class')) selected @endif>Revenue</option>
